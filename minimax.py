        ...
        for x in range(3):
            for y in range(3):
                if board[x][y] == 0:  #空點 下下看
                    ...
                    board[x][y] = 1
                    value = max(value, treeSearch(vertex.child[-1], not player))
                    board[x][y] = 0
        vertex.result = value
        return value
        ...
        for x in range(3):
            for y in range(3):
                if board[x][y] == 0: #空點 下下看
                    vertex.child.append(node(x, y))
                    board[x][y] = 2
                    value = min(value, treeSearch(vertex.child[-1], not player))
                    board[x][y] = 0
        vertex.result = value
        return value
